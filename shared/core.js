...
const VAULT_QUARANTINE_REASONS = {
  ...
  // the manifest's own two (SDD-003). Every OTHER manifest refusal reuses a block sentence above,
  // because the block's word is true of the manifest too — a mismatching digest is a mismatching
  // digest. These two say "file" rather than "block" because that is what they are about.
  'manifest-not-format-2':
    'this file does not say `format: 2`, so Time Turtle cannot tell which grammar it is written in.',
  'manifest-revision-not-last':
    'something follows the revision line, which has to be the last thing in this file — Time Turtle would delete it on the next write, so it stops instead.',
...
// ---- the instance manifest: `Time Turtle.md` (SDD-003) ----
//
// UNDER THE PERSONAL SHAPE THE VAULT IS THE INSTANCE, and this file is what says so. It sits at
// `Settings.vaultPaths.catalog` and carries the whole instance story: the cutover, the catalog,
// the parse settings two machines must agree on, and any pre-vault archive days. SQLite becomes a
// per-machine cache that any machine rebuilds from these bytes.
//
// THE MOTIVATING FAILURE (2026-07-28): a fresh personal install on a new machine stamped its own
// cutover into per-machine SQLite and thereby orphaned the vault's existing TT history — including
// a block TT itself had written. The instance story lived in the wrong place. It lives here now.
//
// IT IS A FORMAT-2 DOCUMENT AND NOTHING NEW. `TT.serializeMd` writes the body and `TT.parseMd`
// reads it back, byte for byte, so there is no second catalog grammar to keep in step and no
// converter — Terje ruled that explicitly. What this codec adds is exactly four things:
//
//   1. A TITLE LINE. `# Time Turtle` rather than `# timesheet`, because this note has a name in
//      Obsidian. `parseMd` ignores every `#` line, so the title is carried here and re-emitted;
//      a human who renames the H1 keeps their name.
//   2. THE HEADER EXTENSION — `cutover`, `timeLogHeading`, `timeSeparator`. The last two are VAULT
//      property rather than machine property: two machines disagreeing about which heading TT's
//      block sits under, or which characters go between two times, corrupt each other's notes.
//      `timeSeparator` carries a `Settings.vaultTimeSeparator` VALUE NAME and never raw
//      characters, the same rule `TT.timeSeparator` states — arbitrary characters must not be able
//      to reach a table cell and split their own row.
//   3. VERBATIM CARRIAGE of header lines this build does not know. Same argument as SB-058's
//      unknown-settings-row rule, and it is the manifest's version of the same danger: under the
//      personal shape this file IS the database, so a newer TT's header dropped by an older TT's
//      read-write cycle is data loss with nothing behind it to restore from.
//   4. ONE TRAILING `revision: N · digest` LINE, whole-file, under the daily block's own trust
//      discipline (present-and-matches → verified, present-and-differs → quarantine, absent →
//      unverified). Whole-file rather than per-section is PROVISIONAL — SDD-003 left it open and
//      SB-045's probe may overrule it. It is one line because there is one document and one
//      writer; four independently-anchored regions would let the file half-agree with itself.
//
// WHAT IT DELIBERATELY LOSES, and it is not an oversight. Prose a human writes INSIDE a section is
// dropped on the next rewrite, because `parseMd` does not see it and this codec does not carry it.
// Header-region prose survives (rule 3 above catches it). SB-045 is the feel-gate on exactly this
// question — "does `Time Turtle.md` read well as an Obsidian note a human occasionally edits?" —
// and widening the carriage before that verdict would be guessing at the answer.
//
// NOT `TT.parseVaultCatalog` (SB-058). That codec is a four-section TABLE note and is neither
// wired to the server nor reached from here; SDD-003 chose format 2 for this file instead, so
// nothing in this section touches it.

/**
 * Every way TT can refuse a MANIFEST that a block reason would describe wrongly — the vocabulary
 * as a runtime value, on the SB-109 discipline (`VaultManifestQuarantineReason` in shared/types.ts
 * is derived from this array, so the type and the list cannot disagree by construction).
 *
 * DELIBERATELY SHORT. The manifest reuses the block vocabulary wherever the block's word is true
 * of it — `crlf-line-endings`, `malformed-revision`, `multiple-revisions`, `digest-mismatch` all
 * mean here exactly what they mean there, and inventing a `manifest-` twin for each would be two
 * names for one fact. These two are the cases where a block reason would LIE.
 */
export const VAULT_MANIFEST_QUARANTINE_REASONS = /** @type {const} */ ([
  // The document does not say `format: 2`. Refused rather than parsed, in BOTH directions: a
  // missing or `format: 1` marker sends `parseMd` down the v1 migration path, which fabricates
  // entry labels out of task ids, and a `format: 3` marker is a grammar this build has not been
  // taught. No block reason covers "the whole document is the wrong format" — `unknown-header` is
  // about one table column.
  'manifest-not-format-2',
  // The revision line is there and readable, but something follows it. The manifest's anchor is
  // TRAILING by definition — everything above it is the payload the digest covers — so a line
  // below it is payload the digest never saw and the rewrite would silently delete.
  // `revision-past-next-heading` is the block's word for the mirror-image fault (the anchor
  // escaped its region) and would read as nonsense on a document with no region.
  'manifest-revision-not-last',
]);

/** The manifest's H1 when the file does not carry one of its own. */
TT.MANIFEST_TITLE = 'Time Turtle';
/**
 * The header keys the manifest OWNS, in emit order, each with its model field. The one home of
 * this grammar: the parser matches against it and the serializer emits from it, so a key cannot
 * be readable and unwritable (or the reverse), which is how a header quietly becomes lossy.
 *
 * `currency`, `language` and `format` are NOT here — they are `TT.serializeMd`'s and are emitted
 * by it. Listing them would give this file a second opinion about bytes format 2 already owns.
 */
const MANIFEST_HEADER_KEYS = /** @type {const} */ (['cutover', 'timeLogHeading', 'timeSeparator']);
/** `key: value`, with the key captured — the shape of every line in the header region. */
const MANIFEST_HEADER_RE = /^([A-Za-z][A-Za-z0-9_-]*):\s*(.*)$/;
/**
 * The payload the whole-file digest covers: every line above the revision line, BLANKS EXCLUDED.
 *
 * ONE definition, called by both the parser and the serializer, for the reason DD-023 half 2
 * states about the block digest — emit and verify hashing two definitions that merely agree today
 * is how every note TT writes quarantines on its own next read.
 *
 * BLANK LINES ARE NOT PAYLOAD. The block's payload is header row + delimiter row + data rows and
 * has no blanks in it by construction (DD-009 consequence 1), so excluding them here is the same
 * rule rather than a new leniency — and it means a human adding a blank line between two sections
 * of a note they are reading does not get told their instance manifest is damaged.
 *
 * Each surviving line still goes through `normaliseVaultPayloadLine`, which passes a non-table
 * line back untouched. That is not decoration: the manifest's archive day sections are bullet
 * rows today, and routing them through the shared normaliser is what keeps this hash the same
 * hash if a table ever lands in this file.
 * @param {string[]} lines every line above the revision line @returns {string}
 */
const manifestPayloadDigest = (lines) => vaultPayloadDigest(lines.filter((line) => line.trim() !== ''));

/**
 * The manifest's bytes. Always emits a digest — the digest-less shape is a READ-side concession to
 * a file TT did not write (DD-009 consequence 2), never an emitter option, exactly as
 * `vaultRevisionLine`'s own ruling says.
 *
 * The revision is NOT bumped here. Serializing is not committing; bumping belongs to whoever
 * decided the catalog changed, the same split `serializeVaultBlock` keeps.
 * @param {import('./types.ts').VaultManifest} manifest
 * @returns {string}
 */
TT.serializeManifest = function (manifest) {
  // The body is format 2's, produced by its own serializer and then extended — never re-emitted
  // here. `serializeMd` writes `# timesheet` and three header lines before the first section, and
  // this splices into that shape rather than restating it.
  const lines = TT.serializeMd(manifest.catalog).split('\n');
  lines.pop(); // the trailing '' left by the final newline
  lines[0] = '# ' + (manifest.title || TT.MANIFEST_TITLE);
  const at = lines.indexOf('format: 2');
  // Not defensive noise: this is the one assumption this function makes about another function's
  // bytes, and a silent -1 would splice the header extension onto the end of the file.
  if (at < 0) throw new Error('serializeManifest: TT.serializeMd emitted no `format: 2` line');
  /** @type {string[]} */
  const header = [];
  for (const key of MANIFEST_HEADER_KEYS) {
    const value = manifest[key];
    // Emit-when-set, the discipline the v2 mirror's ` | archived` token uses: a manifest with no
    // cutover carries no `cutover:` line rather than an empty one that reads as a value.
    if (value != null && value !== '') header.push(key + ': ' + value);
  }
  lines.splice(at + 1, 0, ...header, ...(manifest.extraHeaders || []));
  const revision = manifest.revision == null ? 1 : manifest.revision;
  return lines.concat(['', vaultRevisionLine(revision, manifestPayloadDigest(lines))]).join('\n') + '\n';
};

/**
 * Read a manifest, or refuse. Never returns a partial model: a refusal hands back the reason and
 * nothing else, because half a catalog is worse than none — `TT.rateOf` resolves project → client
 * → rate, so a manifest that kept its projects and dropped its clients invoices every hour at 0.
 * @param {string} md
 * @returns {import('./types.ts').VaultManifestParseResult}
 */
TT.parseManifest = function (md) {
  // Before anything else, and for SB-084's reason: a stray `\r` explains the WHOLE file, and every
  // diagnosis below it would otherwise be a confident lie about a line the human is looking at.
  if (md.indexOf('\r') >= 0) return vaultQuarantine('crlf-line-endings');
  const lines = md.split('\n');
  const fenced = markFences(lines);
  /** @type {number[]} */
  const anchors = [];
  ...
  const malformed = [];
  for (let i = 0; i < lines.length; i++) {
    if (fenced[i]) continue; // a fenced example of the format is documentation, not an anchor
    if (REVISION_RE.test(lines[i])) anchors.push(i);
    else if (MALFORMED_REVISION_RE.test(lines[i])) malformed.push(i);
  }
  if (anchors.length + malformed.length > 1) return vaultQuarantine('multiple-revisions');
  if (malformed.length) return vaultQuarantine('malformed-revision');
  const at = anchors.length ? anchors[0] : -1;
  if (at >= 0 && lines.slice(at + 1).some((line) => line.trim() !== ''))
    return vaultQuarantine('manifest-revision-not-last');
  const payload = lines.slice(0, at >= 0 ? at : lines.length);
  const match = at >= 0 ? REVISION_RE.exec(lines[at]) : null;
  const revision = match ? +match[1] : null;
  const digest = match && match[2] ? match[2] : null;

  /** @type {import('./types.ts').VaultManifest} */
  const manifest = {
    title: TT.MANIFEST_TITLE,
    cutover: null,
    timeLogHeading: null,
    timeSeparator: null,
    extraHeaders: [],
    catalog: /** @type {any} */ (null), // filled below, once the format is known to be one TT reads
    revision,
    digest,
  };
  let format = null;
  // The header REGION is everything above the first `## ` section, which is exactly the region
  // `parseMd` reads its own headers out of (`match && !section`). Reading further would let a
  // `cutover:` line typed inside a day section become the instance's cutover.
  for (let i = 0; i < payload.length; i++) {
    const line = payload[i];
    const trimmed = line.trim();
    if (/^##\s+/.test(trimmed)) break;
    if (!trimmed) continue;
    // A fenced line is content: a note documenting the manifest format is the note most likely to
    // carry a fenced `cutover:` example, and reading it would set the instance's cutover from
    // someone's documentation. Same rule the block locator applies to its anchors.
    if (fenced[i]) {
      manifest.extraHeaders.push(line);
      continue;
    }
    const title = /^#\s+(.+)$/.exec(trimmed);
    if (title) {
      manifest.title = title[1].trim();
      continue;
    }
    const header = MANIFEST_HEADER_RE.exec(trimmed);
    if (!header) {
      manifest.extraHeaders.push(line);
      continue;
    }
    if (header[1] === 'format') {
      format = header[2].trim();
      continue;
    }
    // `currency` and `language` are format 2's own and are read by `TT.parseMd` below; consuming
    // them here keeps them out of `extraHeaders`, where they would be emitted a second time.
    if (header[1] === 'currency' || header[1] === 'language') continue;
    const key = MANIFEST_HEADER_KEYS.find((k) => k === header[1]);
    if (key) manifest[key] = header[2].trim();
    else manifest.extraHeaders.push(line);
  }
  // BEFORE THE DIGEST, and the order is the whole point. A `format: 3` manifest written by a newer
  // TT carries a digest that matches perfectly, so a digest-first order would let this build read
  // a grammar it has not been taught and report it verified. In the other direction the order only
  // improves the diagnosis: a hand-edit from `2` to `1` trips both gates, and "this file does not
  // say format 2" names the edit while "the fingerprint does not match" only names its shadow.
  if (format !== '2') return vaultQuarantine('manifest-not-format-2');
  // The digest gate runs before the PARSE, deliberately. A mismatching manifest is refused whole
  // and its contents are never handed to a caller who might act on them.
  if (digest && digest !== manifestPayloadDigest(payload)) return vaultQuarantine('digest-mismatch');
  manifest.catalog = TT.parseMd(payload.join('\n'));
  return { quarantine: false, manifest, verified: !!digest };
};
