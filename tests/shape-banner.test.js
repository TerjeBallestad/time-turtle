...
const STRANDED_RE =
  /\[time-turtle\]\s+(\d+) entr(?:y|ies) dated before it stays? in SQLite and never reach(?:es)? the vault/;
const RANGE_RE =
  /\[time-turtle\]\s+\((\d{4}-\d{2}-\d{2}) … (\d{4}-\d{2}-\d{2})\); (\d+) commit segments? freezes? with them/;
...
describe('a boot into `personal` says what it stranded before it says what it retired', () => {
  it('prints the stranding lines, with the numbers this data dir actually holds', async () => {
    ...
    expect(Number(range[3])).toBe(want.segments);
  ...
  it('a `team` boot says nothing about stranding at all', async () => {
    ...
    // Give the boot the same window the personal ones get, so this is silence and not earliness.
    await until(() => false, { timeout: 400 });
    const output = server.output();
    await stopServer(server.child);
  ...
  it('an install with nothing to strand stays quiet, and still retires', async () => {
    ...
    // A save with no entries still writes the mirror file, so there IS something to retire.
