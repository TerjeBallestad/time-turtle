...
// plus the end-to-end that actually matters: the frozen figures are read back through
// serializeMd → parseMd, not out of the payload that was sent. The accepting half matters as
// much as the rejecting half —
...
//
// Everything is read back with GET, and through the codec, rather than trusted from the
...
//     'admin rename re-points BOTH users' projects…' fails (`projects` 2 → 0) and the
//     round-trip test fails — the project rows still carry the old join key. (It failed on the
//     mirror bytes when this block owned a mirror test; SB-181 deleted that one with the
//     mirror, and the break is otherwise unchanged.)
