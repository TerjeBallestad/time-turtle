...
// SB-070: an entry id is the one caller-supplied string that reaches the markdown `## commits`
...
// by a round-trip through the codec. Terje ruled option 1: reject the id at the API boundary,
// server-side.
//
...
describe('SB-070 entry-id charset guard', () => {
  ...
  it('rejects the pipe (the SB-070 repro) with 400, and the rejected PUT writes NOTHING', async () => {
    ...
    // hostile id is nowhere near the codec
...
// a round-trip through the codec. Fixed the SB-070 way: reject at the API boundary before
// anything writes.
//
...
//     landed in the store, and serializing the store parsed back to two identical keys with
...
//
// Proven at the api rung: hostile payloads over real HTTP, and the store re-read after each
// rejection — the frozen figures come back out of the STORE, through the admin's full-ledger
// GET (`/api/users/:id/timesheet`, money included), never out of the payload that was sent.
//
// WHAT THIS BLOCK NO LONGER HAS: an end-to-end oracle below the API. Until SB-181 the block
// also owned `the frozen money survives a round-trip through the mirror BYTES on disk`, which
// read the file the server wrote; SB-181 deleted the mirror and that test with it. Nothing here
// calls serializeMd or parseMd now (the only codec assertions in this file are SB-072's and
// SB-075's). So a serializer regression in the unescaped `## commits` section is NOT caught by
// this block — the API guard below is what stands between a `|` and the frozen money, and
// `tests/roundtrip.test.js` is what pins the emitted bytes. Do not read this stanza as
// end-to-end cover.
//
// The accepting half matters as much as the rejecting half —
...
describe('SB-074 commit segment key / committedAt guard', () => {
  ...
  it('rejects every other hostile segment key, including well-formed-but-impossible ones', async () => {
    ...
    const hostile = [
      ...
      ' ' + KEY, // leading space
      KEY + ' ', // trailing space (the codec trims cells — SB-072)
      '', // empty
    ];
...
// SB-072: the markdown parser splits a row on `|` and trims every cell it produces
// (`splitUnescaped(s, '|', true)`, shared/core.js), so it cannot tell format padding from typed
// content — a stored note `'trailing space '` came back `'trailing space'` through the codec.
// Terje's re-ruling: make the DATA fit the format instead of escaping the edges — trim
// `entry.note` and `client.name` on the way in, server-side, so the DB never holds a value the
// codec cannot round-trip. Server-side is the point: the claim is a data-integrity guarantee,
// and a client-side trim is bypassable by exactly the raw PUTs below.
//
// The guard outlived what first motivated it. SB-181 removed the markdown mirror and
// Settings → Markdown, so nothing restores from a file any more; `TT.serializeMd`/`TT.parseMd`
// stay (the demo seed is written and parsed as markdown), and the guarantee is about them.
//
// Proven at the api rung — every assertion below reads what the server STORED, via a fresh GET,
// never the payload that was sent. The interior-whitespace control is load-bearing: `'a  b'` is
// what a careless trim/collapse breaks, and the format genuinely carries it.
//
// ## Verified red-green: 2026-07-25, counts RE-MEASURED 2026-09-17 (SB-181 deleted this block's
// mirror-on-disk test, so the old counts named a test that is gone)
...
//   trimmed' and the serialize→parse test fail (3 of 4 in this block; the interior-whitespace
//   control stays green, as it must). Was 4 of 5 when this block also owned a mirror-on-disk
//   test; SB-181 deleted that test with the mirror, and nothing else about the break changed.
describe('SB-072 edge whitespace is trimmed at the write edge', () => {
  ...
  // whitespace is representable in the markdown format and must survive untouched.
  const NOTES = {
  ...
  it('an interior-whitespace note is not collapsed or trimmed (the control)', async () => {
    // This is what a careless trim breaks, and what the markdown format genuinely does carry.
    const byId = Object.fromEntries((await stored()).entries.map((e) => [e.id, e.note]));
...
// SB-075: SB-072 above made the guarantee — the DB never holds a value the markdown codec
// cannot round-trip — true for `entry.note` and `client.name` only. `project.name`,
// `task.label` and `entry.label` were trimmed in the CLIENT alone (ProjectsSection, TaskModal),
// so a direct PUT still stored edge whitespace that `splitCells` silently ate on the way back
// out. Same bug, same ruling, same layer: three more `.trim()`s in server/src/db.js.
//
// Same shape as SB-072's block, deliberately: raw PUTs over real HTTP with no client code in
// the loop, then every assertion reads what the server STORED, via a fresh GET, never the
// payload that was sent. The `'a  b'` interior control is load-bearing: it is what a careless
// trim/collapse breaks, the format genuinely carries it, and it must stay green straight
// through the break below.
//
// ## Verified red-green: 2026-07-26, counts RE-MEASURED 2026-09-17 (same reason as SB-072 above)
...
//   project.name trimmed', 'stores task.label trimmed', 'stores entry.label trimmed' and the
//   round-trip test all fail (4 of 5 in this block; the interior-whitespace control stays
//   green, as it must). Was 5 of 6 when this block also owned a mirror-on-disk test; SB-181
//   deleted that test with the mirror, and nothing else about the break changed.
describe('SB-075 edge whitespace is trimmed at the write edge for project/task/entry labels', () => {
  ...
  beforeAll(async () => {
    ...
    expect(put.status).toBe(200);
    // Tasks and entries are the caller's own collections. This PUT runs LAST so the state the
    // round-trip serializes carries the new catalog as well as this user's own rows.
...
//
// Everything is read back with GET rather than trusted from the rename's own response: a 200
// proves the request was accepted, never that the store moved. (Until SB-181 the block also
// read the renamed id off the mirror bytes; that test went with the mirror, so the GET is now
// the only oracle here and nothing in this block touches the codec.)
//
// ## Verified red-green: 2026-07-26, break 1's count RE-MEASURED 2026-09-17
//   break 1 — drop the `UPDATE projects SET client_id` from db.renameClientId: 'admin rename
//     re-points BOTH users' projects…' fails (`projects` 2 → 0) — the project rows still carry
//     the old join key. RE-MEASURED 2026-09-17: exactly 1 of the 7 tests in this block, down
//     from 2. The second was the mirror-bytes test SB-181 deleted; the break itself is
//     unchanged.
...
describe('SB-087 server-reconciled client-id rename (SB-067 fix 3)', () => {
  ...
  it('rejects an out-of-charset target before anything writes', async () => {
    // the `|` is the one that matters: a client id is a cell in the markdown `## clients`
