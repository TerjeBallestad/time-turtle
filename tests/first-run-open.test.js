...
//
//   1. THE OPEN STATE. One user, and no stored answer (`firstRunAt`, or a legacy `shape` row). An
//      install that answered has answered, and re-asking would overwrite what its person chose.
//   2. THE PEER SOCKET, and this is the half that actually holds. With no `TT_HOST` the server is on
...
// ## Verified red-green: 2026-09-17 (SB-181: the shape answer became `{ demo }`)
...
/** A fresh install in the OPEN STATE — nothing answered, and no demo hours. */
...
//     FAIL  a body that is not `{ demo }` is refused, and nothing is stored (added by SB-181)
...
//   the surface once the question is answered removed. Fails:
//     FAIL  answering the question closes the surface permanently
...
describe('DD-024 clause 1: the first run answers without a credential', () => {
  ...
  it('answering the question closes the surface permanently', async () => {
    ...
    const answer = await postFirstRun(server.port, { demo: false });
    expect(answer.status).toBe(200);
    expect(answer.json).toEqual({ ok: true, demo: false });
    ...
    const again = await postFirstRun(server.port, { demo: false });
  ...
  it('an install that grew a second user leaves the open state without a restart', async () => {
    // More than one user has ANSWERED BY EXISTING. The count is re-checked per request — an install
    // that adds a second user mid-session leaves the open state immediately, not at the next restart.
    ...
    expect((await postFirstRun(server.port, { demo: false })).status).toBe(409);

    await stopServer(server.child);
  }, 60000);

  // ## Verified red-green: 2026-09-17, TRANSCRIBED. The body check disabled (`if (false && !valid)`):
  //   ×  a body that is not `{ demo }` is refused, and nothing is stored
  //      AssertionError: body {"shape":"team"}: expected 200 to be 400 // Object.is equality
  it('a body that is not `{ demo }` is refused, and nothing is stored', async () => {
    const server = await startServer(open('first-run-bad-body'));

    for (const body of [{ shape: 'team' }, { demo: 'yes' }, { demo: true, shape: 'personal' }, ['demo'], 'demo']) {
      const res = await postFirstRun(server.port, body);
      expect(res.status, `body ${JSON.stringify(body)}`).toBe(400);
    }
    // Still open, so nothing was stored on the way to the refusal.
    expect((await bare(server.port, '/api/first-run')).json.open).toBe(true);
    // …and the install has no demo content either, so the refusal did not seed on its way out.
    const answer = await postFirstRun(server.port, {});
    expect(answer.json).toEqual({ ok: true, demo: false });
...
describe('DD-024 clause 1: the gate is the peer socket, not the Host header', () => {
  ...
  //   clause 1 prices this route class on being "narrow and SELF-CLOSING". SB-181 narrowed the
  //   payload to exactly `{ open, defaultLogin }`; a field added to it reddens this test.
  it('what it still says once the question is over is only what the login screen needs', async () => {
    ...
    const server = await startServer({ ...open('first-run-narrow'), TT_ADMIN_PASSWORD: '' });

    const before = await bare(server.port, '/api/first-run');
    expect(before.json.open).toBe(true);

    expect((await postFirstRun(server.port, { demo: false })).status).toBe(200);

    // `defaultLogin` is the one field that survives, because the moment it is needed IS the moment
    // `open` is false: the answer closes the first run and lands the person on `<Login>`.
    const after = await bare(server.port, '/api/first-run');
    expect(after.status).toBe(200);
    expect(Object.keys(after.json).sort()).toEqual(['defaultLogin', 'open']);
    expect(after.json.open).toBe(false);
  ...
  it('the LAN caller cannot ANSWER the question either', async () => {
    // Read-only is not the claim. A LAN caller that could POST would answer the first run of an
    // install it does not own.
    const lan = lanAddress();
...
//
// WHY IT RIDES ON THIS SURFACE AT ALL. The first run sits in front of the login, so a person who
// finishes it lands on `<Login>` holding a password nobody ever showed them — `seedIfEmpty`
// announces it once on stdout, which `tt serve` redirects into a detached log file.
//
// AND WHY THE GATE IS THE SAME ONE. With no `TT_HOST` the bind is EVERY INTERFACE — an install
// serving the network is what a team install is FOR. So the only thing standing between this credential
...
describe('DD-024 clause 2: the starting-password hint, and who is allowed to read it', () => {
  ...
  it('it retires itself the moment the password changes', async () => {
    ...
    // Answer the first run first, because that is what the hint exists for: it closes the open state
