...
| **unit** | `npm test` (vitest, `tests/`) — the always-green gate; **288 tests at the end of PLAN-019** (branch `remove-obsidian`, the state left for merge) (it was 940 on `main` @ `3767e97`; PLAN-019 deleted the vault, shape and mirror suites with their subject). State the baseline count you started from, and never let it drop except by deleting a test whose behaviour you removed — then account for every one | pure logic: time parsing, ISO weeks, billing rounding, the markdown codec's round-trip | anything HTTP or rendered |
| **api** | `curl` against `localhost:3001` with a cookie jar (login → act → assert JSON/status). Role checks are the canon: employee gets 403 on catalog writes, rates come back null | server behavior incl. auth, roles, persistence, the commit ledger | anything the user sees |
...
**Choosing the browser instrument.** **cmux for _does this read right_** — a visual verdict on a real session, where the answer is a human-readable observation and a cropped screenshot (a committed day's read-only row is the example). **Playwright for _does this still work_** — repeatable, parallel-safe, headless, and, unlike an observation, it can go **red later**. If the check has a definite pass/fail the machine can state, commit it to `test:browser`; if the check is "look at it and tell me", that is cmux, and its output is evidence in a report, not a gate.
...
Playwright lives in `tests-browser/`, **never** in `npm test` — that gate stays fast and always green. Commit only what is structurally invisible below the browser: commit-boundary shaped flows, where a keystroke has to come out the other end as what the server stored, and controls that can lie about their own state (a rejected rename that leaves the refused value on screen). A browser test that only asserts JSON is an api test wearing a DOM — move it to `tests/api.test.js`.

**Neither instrument replaces the `terje` rung.** Playwright can prove the server stores `ballestad-studios`; it cannot prove the settings table feels right. Track F still has exactly one oracle.
