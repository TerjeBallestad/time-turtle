...
| **unit** | `npm test` (vitest, `tests/`) — the always-green gate; **288 tests on `remove-obsidian` @ `5e1a0ab`**, the branch PLAN-019 leaves for merge (it was 940 on `main` @ `3767e97`; PLAN-019 deleted the vault, shape and mirror suites with their subject). State the baseline count you started from, and never let it drop except by deleting a test whose behaviour you removed — then account for every one | pure logic: time parsing, ISO weeks, billing rounding, the markdown codec's round-trip | anything HTTP or rendered |
